...
```{r}
cfu.raw <- read.csv("data/20240811_CFU.csv")
colnames(cfu.raw)[1] <- "time"
```

# Calculate CFU/mL
```{r}
cfu <- cfu.raw %>%
  mutate(cfu.ml.2 = ((cfu.raw$cfu.2/cfu.raw$vol) * 1000)/ 10^-2) %>%
  mutate(cfu.ml.3 = ((cfu.raw$cfu.3/cfu.raw$vol) * 1000)/ 10^-3) %>%
  mutate(cfu.ml.4 = ((cfu.raw$cfu.4/cfu.raw$vol) * 1000)/ 10^-4) %>%
  mutate(cfu.ml.5 = ((cfu.raw$cfu.4/cfu.raw$vol) * 1000)/ 10^-5)

# For WT heat-treated, use cfu.ml.2 to estimate spore densities. 
# For all other samples, use cfu.ml.4
```

# Are there any spores in the mutant strain?
```{r}
mut.spore <- cfu %>% 
  filter(strain == "mutant", heat == "yes") %>%
  summarise(
    sum.10_2 = sum(cfu.ml.2, na.rm = TRUE),
    sum.10_3 = sum(cfu.ml.3, na.rm = TRUE),
    sum.10_4 = sum(cfu.ml.4, na.rm = TRUE),
    sum.10_5 = sum(cfu.ml.4, na.rm = TRUE)
  )
...
wt.veg <- cfu %>%
  ...
  select(time, rep, cfu.ml.4)

wt.veg.sum <- wt.veg %>%
  group_by(time) %>%
  summarise(
    mean.cfu = mean(cfu.ml.4, na.rm = TRUE),
    count = n(),  # Count the number of observations per group
    sem.cfu = ifelse(count > 1, sd(c(cfu.ml.4, na.rm = TRUE)) / sqrt(count), NA)
  )

# wild type spores
wt.spore.early <- cfu %>%
  filter(strain == "wt", heat == "yes", time >= 0, time <= 6) %>%
  select(time, rep, cfu.ml.2)
colnames(wt.spore.early)[3] <- "cfu.ml"

wt.spore.late <- cfu %>%
  filter(strain == "wt", heat == "yes", time >= 7, time <= 13)  %>%
  select(time, rep, cfu.ml.4)
colnames(wt.spore.late)[3] <- "cfu.ml"

wt.spore <- rbind(wt.spore.early, wt.spore.late)
  
wt.spore.sum <- wt.spore %>%
  group_by(time) %>%
  summarise(
    mean.cfu = mean(cfu.ml, na.rm = TRUE),
    count = n(),  # Count the number of observations per group
    sem.cfu = ifelse(count > 1, sd(c(cfu.ml, na.rm = TRUE)) / sqrt(count), NA)
  )
...
spore.efficiency <- ifelse(wt.spore$cfu.ml == 0, NA, wt.spore$cfu.ml / wt.veg$cfu.ml)

mean.spore.efficiency <- mean(spore.efficiency,  na.rm = TRUE)
```
...
```{r}
# Following line initiate location to write file
png(filename ="figures/CFU_20240811.png", 
    width = 1200, height = 1200, res = 96 * 2) 
...
plot(wt.veg.sum$time, wt.veg.sum$mean.cfu, xlim = c(-2, 17), 
     ylim = c(1e7, 3e8), type = "l", 
     lty = 1, col = "blue", lwd = 2, ylab = "", xlab = "", 
     cex.lab = 1.5, las = 1, yaxt = "n", xaxt = "n", log = "y")

# Add points (symbols) on the line
points(wt.veg.sum$time, wt.veg.sum$mean.cfu, pch = 16, col = "blue") 
...
lines(wt.spore.filt$time, wt.spore.filt$mean.cfu, 
      col = "lightblue", lty = 1, lwd = 2)
points(wt.spore.filt$time, wt.spore.filt$mean.cfu, pch = 16, col = "lightblue") 
...
lines(mut.veg.sum$time, mut.veg.sum$mean.cfu, 
      col = "red", lty = 1, lwd = 2)
points(mut.veg.sum$time, mut.veg.sum$mean.cfu, pch = 16, col = "red") 
...
# Add axis ticks and labels
axis(2, at = c(1e3, 1e4, 1e5, 1e6,1e7, 1e8, 1e9), cex.axis = 1.25,
      labels = expression(10^3, 10^4, 10^5, 10^6, 10^7, 10^8, 10^9), las = 1)

axis(4, labels = F, lwd.ticks = 2, las = 1, cex.axis = 1.25,
      at = c(10^3, 10^4, 10^5, 10^6, 10^7, 10^8, 10^9))
...
# Add a legend
legend("bottomright", legend = c("wild type: vegetative", 
...
# Shows plot in R console window
img <- readPNG("figures/CFU_20240811.png")
grid.raster(img)
```
